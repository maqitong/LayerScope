...
def estimate_latency_fields(expert_timing, model, runtime_summary):
    total_gpu = 0.0
    total_cpu = 0.0
    total_preload = 0.0
    total_calls = 0.0
    ...
    gpu_expert_count = (
        ex.static_gpu_hit_count
        + ex.placeholder_hit_count
        + ex.ondemand_load_count
    )

    cpu_expert_count = 0
    sched = runtime_summary.get("schedule")
    if sched is not None:
        for row in sched.get("by_layer", []):
            cpu_expert_count += row.get("cpu_total", 0)

    if cpu_expert_count == 0 and total_calls > 0 and gpu_expert_count > 0:
        num_experts_per_tok = getattr(model.config, "num_experts_per_tok", 6)
        avg_gpu_per_call = gpu_expert_count / total_calls
        cpu_expert_count = max(
            int(total_calls * (num_experts_per_tok - avg_gpu_per_call)), 1
        )

    if total_calls > 0:
        if total_gpu > 0 and gpu_expert_count > 0:
            gpu_avg_ms = (total_gpu / gpu_expert_count) * 1000
        else:
            gpu_avg_ms = model.latency_gpu
        if total_cpu > 0 and cpu_expert_count > 0:
            cpu_avg_ms = (total_cpu / cpu_expert_count) * 1000
        else:
            cpu_avg_ms = model.latency_cpu
        preload_calls = ex.preload_success_count
    ...
    if total_gpu > 0 and gpu_expert_count == 0:
        fallback_notes.append("gpu_expert_count=0 but gpu timing>0; gpu_avg may be inaccurate")
    if total_cpu > 0 and cpu_expert_count == 0:
        fallback_notes.append("cpu_expert_count=0 but cpu timing>0; cpu_avg may be inaccurate")
...
if __name__ == "__main__":
    ...
    latency_estimates = estimate_latency_fields(expert_timing, model, runtime_summary)
