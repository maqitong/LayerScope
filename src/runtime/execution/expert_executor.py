...
from common.types import ExpertLayerContext, ExpertSchedule


class ExpertExecutionManager:
    def __init__(self, device, placeholder_manager, expert_provider=None, is_expert_in_gpu: Callable[[int, int], bool] = None, profile_timing: bool = False, cpu_experts=None, model=None):
        self.device = device
    ...
    def get_cpu_expert(self, layer: int, expert_id: int):
        expert = self.cpu_experts.get((layer, expert_id))
        if expert is not None:
            return expert
        if self.expert_provider is not None:
            return self.expert_provider.get_expert(layer, expert_id)
        if self.model is not None:
            return self.model.layers[layer].mlp.experts[expert_id]
        raise RuntimeError("No expert provider available")
    ...
    def _preload_experts_impl(self, demands) -> None:
        ...
        for layer, expert_ids in by_layer.items():
            if self.expert_provider is not None:
                if layer >= self.expert_provider.num_layers:
                    continue
            elif self.model is not None and layer >= len(self.model.layers):
                continue
            tick = time.time()
            loaded = 0
            for expert_id in expert_ids:
